...
# adaptado de https://medium.com/@nicholas.w.swift/easy-a-star-pathfinding-7e6689c7f7b2
def astar(matriz, inicial, final): #aceitar 
    ...
    while len(lista_aberta) > 0:
        ...
        # Generate filhos
        filhos = []
        for new_pos in vizinhos: # Adjacent squares
            # Get node pos
            pos_node = (node_atual.pos[0] + new_pos[0],
                        node_atual.pos[1] + new_pos[1])

            # Make sure within range
            if pos_node[0] > (len(matriz) - 1) or \
               pos_node[0] < 0 or \
               pos_node[1] > (len(matriz[len(matriz) - 1]) - 1) or \
               pos_node[1] < 0:
                continue

            # Make sure walkable terrain
            if matriz[pos_node[0]][pos_node[1]]: continue
        ...
        for filho in filhos:
            ...
            filho.h = ((filho.pos[0] - node_final.pos[0])**2) + (
                       (filho.pos[1] - node_final.pos[1])**2)
